...
public class GestorUsuario {
	...
	public Usuario login(String user, String password) {
		...
		try {
			...
			rs = st.executeQuery("SELECT * FROM usuarios WHERE usuario = '" + user + "' AND password = '" + password + "'");
	...
	public void insertarUsuario(Usuario usuario) {
		...
		try {
			con = gestorConexion.crearConexion();
			st = con.createStatement();
			st.execute("INSERT INTO usuarios (nombre, apellidos, email, telefono, dni, usuario, password) VALUES ('" + usuario.getNombre() + "','"+usuario.getApellidos()+"', '"+usuario.getEmail()+"', '"+usuario.getTelefono()+"', '"+usuario.getDni()+"', '"+usuario.getUsuario()+"', '"+usuario.getPassword()+"' )");
		}catch (SQLException e) {
			e.printStackTrace();
		}finally {
